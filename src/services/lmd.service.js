...
const getLastDataLmdDamOverview = async (user) => {
  try {
    ...
    if (
      ...
      const getLastDataLmdDamPondLevelOverview =
        await LMD_POND_LEVEL_OVERVIEW.findOne()
          .select(
      ...
      const getLastDataLmdDamOverviewPos = await LMD_DAM_OVERVIEW_POS.findOne()
        ...
        .sort({ dateTime: -1 });
      const getLastDataLmdDamOverviewDish =
        await LMD_DAM_OVERVIEW_DICH.findOne()
          .select(
          ...
          .sort({ dateTime: -1 });
      const getLastDataLmdHrDamOverviewPos =
        await LMD_HR_DAM_OVERVIEW_POS.findOne()
          .select("hrrGate1Position hrrGate2Position")
          .sort({ dateTime: -1 });
      const getLastDataLmdHrDamOverviewDish =
        await LMD_HR_DAM_OVERVIEW_DICH.findOne()
...
const lmdDischargeGateReportWp = async (
  ...
  try {
    ...
    if (exportToExcel == 1) {
      ...
      lmdDischargeGateReport1.forEach((row) => {
        const formattedDate = new Date(row.dateTime)
          .toISOString()
          .replace("Z", "");
        csvStream.write({
      ...
      res.setHeader(
        "Content-Disposition",
        "attachment; filename=LMD_Dam_Gate_1_To_20_Discharge_Report.csv"
      );
      ...
      try {
        const dynamicHtml = await ejs.renderFile(
          path.join(__dirname, "../../views/lmdDischargeGate.ejs"),
          {
        ...
        res.setHeader(
          "Content-Disposition",
          "attachment; filename=LMD_Dam_Gate_1_To_20_Discharge_Report.pdf"
        );
...
const lmdOpeningGateReportWp = async (
  ...
  try {
    ...
    if (exportToExcel == 1) {
      ...
      res.setHeader(
        "Content-Disposition",
        "attachment; filename=LMD_Dam_Gate_1_To_20_Opening_Report.csv"
      );
      ...
      try {
        const dynamicHtml = await ejs.renderFile(
          path.join(__dirname, "../../views/lmdOpeningGate.ejs"),
          {
            lmdOpeningGateReportWithoutPagination:
              lmdOpeningGateReportWithoutPagination,
          }
        );
        ...
        res.setHeader(
          "Content-Disposition",
          "attachment; filename=LMD_Dam_Gate_1_To_20_Opening_Report.pdf"
        );
        res.setHeader("Content-Type", "application/pdf");
        res.send(pdfBuffer);
      } catch (error) {
...
const lmdPondlevelGateReportWp = async (
  ...
  try {
    ...
    if (exportToExcel == 1) {
      ...
      res.setHeader(
        "Content-Disposition",
        "attachment; filename=LMD_Dam_Inflow_Outflow_PondLevel_Report.csv"
      );
      ...
      try {
        const dynamicHtml = await ejs.renderFile(
          path.join(__dirname, "../../views/lmdInflowOutflowPondLevel.ejs"),
          {
            lmdPondlevelGateReportsWithoutPagination:
              lmdPondlevelGateReportsWithoutPagination,
          }
        );
        ...
        res.setHeader(
          "Content-Disposition",
          "attachment; filename=LMD_Dam_Inflow_Outflow_PondLevel_Report.pdf"
        );
        res.setHeader("Content-Type", "application/pdf");
        res.send(pdfBuffer);
      } catch (error) {
...
const lmdGateParameterOverviewReportWp = async (
  ...
  try {
    ...
    if (exportToExcel == 1) {
      ...
      res.setHeader(
        "Content-Disposition",
        "attachment; filename=LMD_Dam_Parameter_Overview_Report.csv"
      );
      ...
      try {
        const dynamicHtml = await ejs.renderFile(
          path.join(__dirname, "../../views/lmdParameterOverview.ejs"),
          {
            lmdGateParameterOverviewReportWithoutPagination:
              lmdGateParameterOverviewReportWithoutPagination,
          }
        );
        ...
        res.setHeader(
          "Content-Disposition",
          "attachment; filename=LMD_Dam_Parameter_Overview_Report.pdf"
        );
        res.setHeader("Content-Type", "application/pdf");
        res.send(pdfBuffer);
      } catch (error) {
...
const lmdGateReportWp = async (
  ...
  try {
    ...
    if (exportToExcel == 1) {
      ...
      res.setHeader(
        "Content-Disposition",
        "attachment; filename=LMD_HR_Gate_Report.csv"
      );
      ...
      try {
        const dynamicHtml = await ejs.renderFile(
          path.join(__dirname, "../../views/lmdHrGate.ejs"),
          {
        ...
        res.setHeader(
          "Content-Disposition",
          "attachment; LMD_HR_Gate_Report=.pdf"
        );
        res.setHeader("Content-Type", "application/pdf");
        res.send(pdfBuffer);
      } catch (error) {
