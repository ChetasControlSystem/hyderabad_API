...
const Kadam = require("./models/kadam")
...
async function kadamData(data) {
  try {
    const result = data.kadamPondLevel; // Assuming 'data' is the object returned from SRSPDAM
    const result1 = data.result1;
    const mappedData = result.map(row => ({
      DateTime: row.DateTime,
      potourInflowLevel: row.D1,
      potourInflowLevel: row.D2,
      chintakuntaInflowLevel: row.D3,
      gagillapurInflowDischarge: row.D4,
      potourInflowDischarge: row.D5,
      chintakuntaInflowDischarge: row.D6,
      alugunuruBridgeOutflowLevel: row.D7,
      alugunuruBridgeOutflowDischarge: row.D8,
      hrDownstreamLevel: row.D9,
      hrDownstreamDischarge: row.D10,
      D11: row.D11,
      D12: row.D12,
      D13: row.D13,
      D14: row.D14,
      ...
      cathmentArea: row.D19,
      contourArea: row.D20,
      ayucutArea: row.D21,
      fillingPercetage: row.D22,
      fullReserveWater: row.D23,
      instGateDischarge: row.D24,
      instCannalDischarge: row.D25,
    ...
    const lastData = await Kadam.find().sort({ DateTime: -1 }).limit(1);

    if (lastData.length) {
      const LastDate = new Date(lastData[0].DateTime);
      ...
      await Kadam.insertMany(newArray);
    } else {
      await Kadam.insertMany(mappedData);
    }
  } catch (error) {
    console.error('Error handling MongoDB data:', error);
    throw error;
...
module.exports = { connectToMongoDB, handleMongoDBData, kadamData };
